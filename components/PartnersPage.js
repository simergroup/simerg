...
function Carousel({ partners }) {
	...
	useEffect(() => {
		if (partnerParam !== null) {
			const index = parseInt(partnerParam);
			if (!isNaN(index) && index >= 0 && index < partners.length) {
				setCurrentIndex(index);
			}
		}
	}, [partnerParam, partners.length]);

	const handlePrev = () => {
		setCurrentIndex((prevIndex) => (prevIndex === 0 ? partners.length - 1 : prevIndex - 1));
	};
		...
		<div className="relative w-full overflow-hidden">
			...
			>
				{partners.map((partner) => (
					<div key={partner._id} className="min-w-full rounded-lg bg-neutral-800 p-4">
